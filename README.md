...
| **Identity & profiles** | Optional Google sign-in. Signing in gets you a **generated nickname** — never your real name, which is never copied out of the auth layer — plus a badge wall, per-mode personal bests, and a leaderboard row that carries your handle. Since profiles-v2 your **history follows the profile across browsers**: the hall of fame (company names included), the lifetime companies-built counter, badges and bests all merge two-way on every sign-in — union by run identity, so no device can erase another's runs. Anonymous play is untouched and always will be. |
